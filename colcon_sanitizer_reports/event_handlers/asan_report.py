...
class ASanReportEventHandler(EventHandlerExtensionPoint):
    ...
    def __call__(self, event):
        ...
        if isinstance(data, TestFailure):
            jobs = event[1]
            self._start_asan_output_parsing(jobs)

    def _start_asan_output_parsing(self, jobs):
        input_file = get_log_path() / jobs[0].identifier / \
            log.STDOUT_STDERR_LOG_FILENAME
        output_file = 'asan_report_' + str(date.today()) + '.xml'
...
def main():
    ...
    with open(
            ...
            'colcon_sanitizer_reports/tsan.log', 'r') as asan_f_in:
        report = Report(asan_f_in.readlines())
    ASanReportEventHandler()._convert_log_to_xml(
